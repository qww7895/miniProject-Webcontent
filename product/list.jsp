	...
	<div id="container">
		<aside>
			...
			<center>
				...
				<div class="dep2" style="display:block">
					<a href="list.pb?pid=PDC00000034" <%if(category.equals("")){%> class="on"<% }else{%>class =""<% } %>>전체보기</a>
					<a href="list.pb?pid=PDC00000034&s_category=PDC00000029" <%if(category.equals("PDC00000029")){%> class="on"<% }else{%>class =""<% } %>>카페 아다지오</a>
					<a href="list.pb?pid=PDC00000034&s_category=PDC00000030" <%if(category.equals("PDC00000030")){%> class="on"<% }else{%>class =""<% } %>>라떼류 & 티</a>
					<a href="list.pb?pid=PDC00000034&s_category=PDC00000048" <%if(category.equals("PDC00000048")){%> class="on"<% }else{%>class =""<% } %>>주스 & 에이드</a>
					<a href="list.pb?pid=PDC00000034&s_category=PDC00000047" <%if(category.equals("PDC00000047")){%> class="on"<% }else{%>class =""<% } %>>스무디</a>
					<a href="list.pb?pid=PDC00000034&s_category=PDC00000046" <%if(category.equals("PDC00000046")){%> class="on"<% }else{%>class =""<% } %>>빙수</a>
					<a href="list.pb?pid=PDC00000034&s_category=PDC00000031" <%if(category.equals("PDC00000031")){%> class="on"<% }else{%>class =""<% } %>>완제음료</a>				</div>
		...
		<div class="text_title_01">전체상품보기
			<p>
<!-- 				<i><a href="product/list.pb?orderby=reg&pid=PDC00000004" class="on">최신등록순</a></i>
				<i><a href="product/list.pb?orderby=hit&pid=PDC00000004" class="">조회순</a></i>
				<i><a href="product/list.pb?orderby=like&pid=PDC00000004" class="">좋아요순</a></i> -->
			</p>
		</div>
		<div class="product_list_style_01">
			<ul>
				...
				<!--  -->
				<%
				List pblist=(List)request.getAttribute("productlist");
				for(int i=0;i<pblist.size();i++){
					ProductBean beans = (ProductBean)pblist.get(i);
					%>
				<li style="display:none;">
					...
					<div class="like">
						...
						<span><%=beans.getPLIKE() %></span>
					</div>
					<img src="data/product/<%=beans.getPPICTURE() %>" alt="<%=beans.getPNAME() %>" width="220px" height="220px" />
					<p><%=beans.getPNAME() %></p>
					<div class="product_info">
						<a href="view.pb?id=<%=beans.getPINDEX() %>&pid=<%=beans.getPBCLASS() %>" class="text_info">
							<strong><%=beans.getPNAME() %></strong>
							<p><%=beans.getPINTRO() %></p>
						</a>
						<div class="like">
